...
namespace {
    ...
    class logcat {
      public:
        ...
        bool operator()(const fostlib::log::message &m) const {
            ...
            jobject message = env->NewStringUTF(
                    fostlib::json::unparse(m.body(), false).c_str());
            env->CallStaticVoidMethod(
                    crashlytics, crashlytics_logcat, level, tag, message);
    ...
    const fostlib::jni_onload g_loader([](JNIEnv *env) {
        crashlytics = reinterpret_cast<jclass>(env->NewGlobalRef(
